...
- **Minimise cognitive complexity** (the *understandability* metric, distinct from cyclomatic). It scores how hard code is to *follow* via three rules: ignore shorthand → **+1 per break in linear flow** → **+1 extra per nesting level**. Lower a function's score with these levers:
  ...
  - Enforced by a **`complexipy` gate** (threshold 12 in `[tool.complexipy]`, **ratchet / new-code-only** via pre-commit) — Ruff's `C901` is cyclomatic and does *not* catch this. One function is still over 12 and grandfathered (`parse_verification_response_v2`=13); don't add new ones or push it higher. (Два колишні порушники розібрані на хелпери: `IngestionOrchestrator::_process_channel` 19 → 6, `PredictionExtractor::extract` 15 → 6.) Audit: `complexipy src`.
