...
- **Minimise cognitive complexity** (the *understandability* metric, distinct from cyclomatic). It scores how hard code is to *follow* via three rules: ignore shorthand → **+1 per break in linear flow** → **+1 extra per nesting level**. Lower a function's score with these levers:
  - **Flatten nesting** — each level is charged progressively, so a nested `if`/`for` costs far more than a flat one. Use guard clauses / early returns (an early `return` is free); lift a nested block into an evocatively-named helper (method extraction is free "shorthand").
  - **One dispatch beats an `if`/`elif` chain** — a `match` / dict-dispatch on one variable is a single increment; an `if`-`elif` chain is charged per branch.
  - **Keep a boolean run single-operator** — `a and b and c` is one increment, but switching between `and`/`or` is charged per run; factor mixed conditions into named booleans/helpers.
  ...
  - Enforced by a **`complexipy` gate** (threshold 12 in `[tool.complexipy]`, **ratchet / new-code-only** via pre-commit) — Ruff's `C901` is cyclomatic and does *not* catch this. Two functions are still over 12 and grandfathered (`extract`=15, `parse_verification_response_v2`=13); don't add new ones or push these higher. (`IngestionOrchestrator::_process_channel` was 19 — розібраний на `_process_post`/`_save_post`/`_embed_all` і тепер 6.) Audit: `complexipy src`.
