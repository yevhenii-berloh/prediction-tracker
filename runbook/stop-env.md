...
## Однією командою (скрипти)

```bash
./deploy/stop.sh               # пауза, з підтвердженням
./deploy/stop.sh -y            # без підтвердження
./deploy/start.sh              # підйом (RDS → бокс, з очікуванням), друкує новий IP
./deploy/status.sh --no-ssh    # перевірити стан (PAUSED / UP)
```

Скрипти резолвлять id-и динамічно, пропускають уже зупинені/підняті ресурси й
тримають правильний порядок підйому. Далі — покроково, коли треба контроль або
скрипт недоступний.
