...
CITATION_SYSTEM = (
    ...
    "на яке це речення посилається. Питання одне: чи саме з цього джерела взято це речення. "
    "\n\n"
    "КЛЮЧОВЕ ПРАВИЛО. Поле status у джерелі — це ВЕРДИКТ верифікатора, і він є достатнім "
    "доказом результату прогнозу. Це стосується ОБОХ напрямків: status=refuted повністю "
    "підтверджує речення «прогноз не справдився», status=confirmed — «прогноз справдився», "
    "unresolved — «оцінити не вдалося», premature — «ще зарано судити». "
    "НЕ вимагай, щоб джерело містило зовнішній доказ провалу чи справдження: доказова база "
    "живе у верифікатора, а не в тексті прогнозу. Речення з вердиктом, що відповідає полю "
    "status, ЗАВЖДИ supported=true. "
    "\n\n"
    "supported=false став лише тоді, коли речення говорить про ІНШИЙ прогноз, іншу тему або "
    "містить факт, якого в джерелі немає й він не випливає з його полів. "
