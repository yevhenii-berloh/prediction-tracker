...
def build_query_prompt(row: dict, source_field: str) -> str:
    """Промпт бачить ПОВНИЙ контекст прогнозу (для якоря), а source_field задає лише акцент
    запиту. Інструкція: зберігай конкретні якорі (субʼєкт/подія/абсолютний період), але
    перефразовуй формулювання — щоб запит був специфічний і реалістичний, не розмитий."""
    return (
        "Ти формуєш пошуковий запит, який пересічний користувач написав би, "
        "щоб знайти САМЕ цей прогноз.\n\n"
        ...
        "(виведи його з дати прогнозу, напр. «наприкінці 2021», а НЕ «найближчим часом»);\n"
        "- але НЕ копіюй формулювання дослівно — перефразуй своїми словами;\n"
        "- уникай загальників («найближчим часом», «нова допомога», «певні події»).\n\n"
        "Приклади:\n"
        "✗ «що чекати від війни найближчим часом?» → "
        "✓ «який розвиток війни прогнозували наприкінці 2021?»\n"
        "✗ «чи змінить ситуацію нова допомога партнерів?» → "
        "✓ «чи вплине постачання західних ППО на фронт навесні 2023?»\n\n"
