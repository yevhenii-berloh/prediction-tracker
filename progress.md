...
**Мінімальний деплой («воно живе»)** — один EC2 + Docker Compose (Postgres+pgvector+app), SSH-only доступ, секрети в приватному S3, інфра як CloudFormation — **реалізовано в коді** (гілка `feat/minimal-aws-deploy`). RDS (Task 23), автоматизація/CI (Task 20) лишаються 📋 QUEUED. Деталі: [`docs/aws-deploy/`](docs/aws-deploy/).
...
**Статус:** дослідження проведено; оформлено як Brain-вікі `wiki/concepts/hybrid-rag-search.md` (**2026-07-10**, 11 findings, цитовано). Дистильовані висновки лишаю тут як швидкий довідник (сирий звіт був у ефемерному воркфлоу-транскрипті; повний відновлено з журналу воркфлоу). **Впровадження в код — ще ні** (це retrieval-трек Частина B).
