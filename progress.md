...
**Оновлення 2026-07-18:** деплой більше не queued — система відпрацювала **перший повний прод-прогін
верифікації** на AWS (2026-07-16): 4116/4116 прогнозів верифіковано, `failed=0`. Деталі й застереження —
у Notes. Незакрите на проді: прогін інжесту (курсор стоїть на 2026-07-14) і його автоматизація.
...
- **Перший повний прод-прогін верифікації (2026-07-16).** Пайплайн уперше пройшов корпус цілком на живій AWS/EC2-інфрі, не локально й не на семплах. Лог боксу, 07:30:18Z: `verification done: verified=2111 failed=0 skipped=0` — нуль падінь, нуль пропусків; прогрес логувався кожні 50. Зріз прод-RDS на 2026-07-18 (`deploy/psql.sh --stats`): **2238 зібраних документів, 4116 видобутих прогнозів, усі 4116 з `verified_at`** — верифікація покрила корпус на 100%; разом 6354 записи по одному автору (Арестович, `@O_Arestovich_official`). Розподіл вердиктів: `unresolved` 1894 (46%), `confirmed` 1064 (26%), `refuted` 764 (19%), `premature` 394 (9.6%) — сума сходиться з 4116. Тим самим шлях `deploy/verify.sh` → `POST /verify/run` підтверджено на живій інфрі, і це перший еталон розподілу статусів на повному корпусі (базова лінія для recheck-лупа: 394 `premature` — саме та популяція, яку він має перечитувати). **Три застереження, щоб число не читалось ширше, ніж воно є:** (1) доступне вікно логу покриває прогін на 2111 прогнозів — решту до 4116 закрив раніший прогін, що у вікно не потрапив, тож усі 4116 одному запуску не приписуються; (2) курсор інжесту стоїть на `2026-07-14 18:00:22Z` (lag ~3.7 дня), тобто успіх стосується **верифікації, не збору** — прод-прогін `ingest.sh` досі за користувачем; (3) `raw_documents.processed = 0` для всіх 2238 документів — див. Tech debt нижче.
