...
> **As-built розійшовся з планом (2026-07-20, коміт `ba1f8b2`).** Рев'ю знайшло, що `latency_ms`
> рахувався **до** запису в БД, а `elapsed` у лог-рядку — **після**, тож два числа про той самий
> запит не збігались. Після фіксу: вимір знімається **один раз** одразу після відповіді оркестратора,
> обидва числа з нього; хелпер бере готовий `latency_ms: int` замість `started: float` і не знає
> про годинник; названий `_record_query`, а не `_log_query` — він робить довговічний запис у БД,
> а «log» у цьому файлі вже означає рядок у процесний лог. Код нижче — як планувалось, не як вийшло.
