...
| Р3  | Автор поза корпусом → **явна відмова із зазначенням автора** («немає інформації про N»), не тихе скидання фільтра | Спитали про Портникова — відповідь про Арестовича гірша за «не знаю»; а генерична відмова приховує причину. Юзер має зрозуміти: справа в авторі, не в темі |
| Р4  | Збій/нерозпарсюваність планера → **помилка запиту** (fail fast)                             | Тихий fallback маскує проблему: запит із фільтр-інтентом без фільтрів мовчки віддає неправильні результати. Аварійний обхід — `query_planner_enabled=False` |
...
class SearchFilters(BaseModel):
    ...
    unknown_author: str | None = None   # ім'я, як згадано в питанні
...
- `unknown_author` — автора названо, але його нема серед відомих персон;
  несе ім'я, як воно згадане в питанні. Сигнал для явної відмови (Р3), не предикат.
  Взаємовиключний із `person_id`.

Розширення `QueryResult` (наявна модель): нове поле `unknown_author: str | None = None` —
прокидає сигнал із фільтрів до answer-шару, який формулює відмову. Опційне з
дефолтом → зворотно сумісне для наявних консумерів.
...
5. Будь-який збій кроків 3–4 → підняти `QueryPlanningError` (fail fast, Р4).
   Виняток пропагується через `QueryOrchestrator.search` до ендпоінта → HTTP 500.
   Планер нічого не ковтає і не логує сам (логує boundary — політика логів).

`QueryPlanningError` — новий доменний виняток (модуль `query/planner.py`).

### 5.3 Промпт-контракт (`llm/prompts.py`)

- `SELF_QUERY_SYSTEM` + `build_self_query_prompt(question, persons, today)`.
- Вихід LLM — JSON з полями `semantic_query`, `person_id`, `unknown_author`,
  чотири дати (ISO або null).
- Інструкції промпта:
  - «коли сказано» → `prediction_date_*`; «про який час прогноз» → `target_date_*` (Р1);
  - автор згаданий і є в списку → `person_id` зі списку, `unknown_author=null`;
  - автор згаданий, але його нема в списку → `unknown_author=<ім'я, як згадано>`,
    `person_id=null`;
  - автор не згаданий → обидва поля null;
...
3. `plan.filters.unknown_author` → одразу
   `QueryResult(results=[], unknown_author=<ім'я>)` — без embed і пошуку.
...
### 5.6 `AnswerOrchestrator` (`query/answer_orchestrator.py`)

Одна нова гілка в `answer()` — між `search` і `answer_from_sources`:

- `result.unknown_author` заповнено → повернути `AnswerResult` з
  `REFUSAL_UNKNOWN_AUTHOR`-шаблоном, підставивши ім'я
  («Немає інформації про автора N…»), `sources=[]`, **без виклику LLM** —
  за зразком наявного `REFUSAL_NO_DATA` short-circuit.
- Нова константа `REFUSAL_UNKNOWN_AUTHOR` поряд із `REFUSAL_NO_DATA`
  (точне формулювання — у плані; той самий тон і дисклеймер-рядок).
- `answer_from_sources` не змінюється (generate-only шлях лишається чистим).

### 5.7 Що НЕ змінюється

- `/query`- і `/answer`-ендпоінти, наявна refusal-логіка на порожніх sources
  (`REFUSAL_NO_DATA`), `answer_from_sources`.
- Схема БД, міграції, embedding-колонка, інжест.

### 5.8 Wiring (`factory.py`, `config.py`)
...
NL-питання
  → AnswerOrchestrator.answer
    → QueryOrchestrator.search
        ...
        → unknown_author? ──► QueryResult(unknown_author) ──► REFUSAL_UNKNOWN_AUTHOR
...
|------|-------------|-----------|
| LLM-виклик планера кинув виняток | `QueryPlanner.plan` | `QueryPlanningError` → пропагується до ендпоінта → HTTP 500 (Р4, fail fast) |
| Відповідь LLM не парситься / не валідна JSON-схема | `parse_query_plan` → `plan` | те саме |
| `*_from > *_to` (інвертований діапазон) | `parse_query_plan` | те саме — невалідний план, не «майже валідний» |
| Автор згаданий, але відсутній у списку персон | планер (за контрактом промпта) | `unknown_author=<ім'я>` → явна відмова «немає інформації про автора N» (Р3). Семантичний результат, не збій |
| `person_id` від LLM нема серед відомих | `parse_query_plan` (валідація проти переданого списку) | `QueryPlanningError` — LLM порушив контракт (id поза списком = невалідний план) |
| Порожній `semantic_query` | `parse_query_plan` | підставити оригінальне питання — детермінована нормалізація, не збій |
| 0 результатів після фільтрів | наявний потік | порожній `QueryResult` → `REFUSAL_NO_DATA` (Р3) |

Межа збій/не-збій: невалідний вихід планера — збій (fail fast); валідний план із
семантикою «нема даних» (невідомий автор, 0 результатів) — легітимна відмова.

Логування: `QueryPlanningError` логується один раз на boundary (не в планері —
без log-and-raise); на успішний план — DEBUG із витягнутими фільтрами
(без тексту питання в INFO — політика логів).
...
- **Юніти (fakes, без мережі):**
  - `parse_query_plan`: валідний план; зламаний JSON → `QueryPlanningError`;
    інвертовані дати → `QueryPlanningError`; `person_id` поза списком →
    `QueryPlanningError`; `unknown_author` проходить як є; порожній
    `semantic_query` → нормалізація.
  - `build_self_query_prompt`: персони в промпті; сьогоднішня дата; схема полів.
  - `QueryPlanner` + FakeLLM: happy path; виняток LLM → `QueryPlanningError`.
  - `QueryOrchestrator` + фейк-планер: фільтри доходять до `search_similar`;
    `unknown_author` → `QueryResult(results=[], unknown_author=<ім'я>)` без embed;
    `planner=None` → стара поведінка (регресія).
  - `AnswerOrchestrator`: `result.unknown_author` → `REFUSAL_UNKNOWN_AUTHOR`
    з ім'ям, LLM не викликається; порожні sources без unknown_author →
    `REFUSAL_NO_DATA` як досі (регресія).
...
- **Смоук (реальна БД + LLM, вручну):** 4 запити — «автор + рік сказання»,
  «прогнози на рік» (target), питання без фільтрів, невідомий автор → явна
  відмова з ім'ям автора. Прогін через `/answer` на локальному compose.
...
- **Планер на критичному шляху (Р4):** будь-який його збій = збій запиту.
  Свідомий трейд-оф на користь прозорості; аварійний обхід —
  `query_planner_enabled=False` (пошук без фільтрів, явно ввімкнений оператором).
