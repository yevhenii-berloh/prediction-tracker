...
| `person_id` від LLM нема серед відомих | `parse_query_plan` (валідація проти переданого списку) | `author_unknown=True` → відмова. Це семантичний результат («такого автора не знаємо»), не збій |
...
- **Юніти (fakes, без мережі):**
  - `parse_query_plan`: валідний план; зламаний JSON → `QueryPlanningError`;
    інвертовані дати → `QueryPlanningError`; невідомий `person_id` →
    `author_unknown`; порожній `semantic_query` → нормалізація.
