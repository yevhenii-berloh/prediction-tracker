...
- Конфіг: нове поле `bot_enabled` (поле `telegram_bot_token` уже є в
  `Settings` і `.env.example` — зарезервоване при скафолді); wiring у
  `factory.py` / `app.py`. aiogram теж уже в `pyproject.toml` — залежності
  не змінюються.
- Юніт-тести хендлерів на фейках; ручний смоук на живому боті.
- Рансетап: `TELEGRAM_BOT_TOKEN` у секрети, `runbook/bot.md`.
...
- **Конфіг (`config.py`):** нове поле `bot_enabled: bool = False`; токен
  читається з наявного `telegram_bot_token` (порожній рядок = відсутній).
  Дефолт вимкнено — локальний дев і тести не зачеплені.
...
2. `TELEGRAM_BOT_TOKEN` уже в `.env.example` — виділити з колекторської
   секції окремим коментарем; додати `BOT_ENABLED=false` і покласти токен
   у S3-секрети бокса.
