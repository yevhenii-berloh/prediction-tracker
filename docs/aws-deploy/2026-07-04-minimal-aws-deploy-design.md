...
**Обсяг зміни в коді.** Єдина правка application-коду — додати config-поле `APP_HOST`
(нове поле `Settings` у `config.py`) і зчитувати його в `__main__.py` при старті
uvicorn. Більше нічого: DB-хост і решта — через `.env`.
